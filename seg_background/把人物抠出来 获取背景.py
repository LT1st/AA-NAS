...
# 分割结果的保存文件夹路径
output_folder = "background_without_human/random_patched"
...
for filename in os.listdir(input_folder):
    if filename.endswith(".png"):
        ...
        input_tensor.save(os.path.join(output_folder, filename))
        ...
        # 保存背景去除人的图像
        input_image_masked = 255 - input_image_masked * 255
        # input_image_masked = input_image_masked.permute(1, 2, 0)[:, :, [2, 1, 0]].contiguous()
        input_image_masked = transforms.functional.to_pil_image(input_image_masked)
        # r, g, b = input_image_masked.split()
        # input_image_masked = Image.merge('RGB', (g, r, b))
