...
class DefaultReproducePaperObj():
    """
...
class MaxProbExtractor(nn.Module):
    ...
    patch_config.ReproducePaperObj 中的參數應該直接写死 全文都用的这个
    4.18修改bug：重构后缺失参数

    Module providing the functionality necessary to extract the max class probability for one class from YOLO output.

    """
    """config is nneded when 
    ~/Code/ad-yolo/load_data.py in forward(self, YOLOoutput)
     58         confs_if_object = output_objectness #confs_for_class * output_objectness
     59         confs_if_object = confs_for_class * output_objectness
---> 60         confs_if_object = self.config.loss_target(output_objectness, confs_for_class)
    """
    def __init__(self, cls_id, num_cls, config="default"):
        super(MaxProbExtractor, self).__init__()
        self.cls_id = cls_id
        self.num_cls = num_cls
        if config=="default":
            thiscfg = DefaultReproducePaperObj()
            self.config = thiscfg
        else:
            self.config = config
    ...
    def forward(self, YOLOoutput):
        ...
        '''
        理论上YOLOoutput输出一个[batch, 5, 85, 19, 19]
        5指的是5个anchor boxes（默认）
        85是每个格子的输出向量，由bx，by，bw，bh，pc和80个分类的预测概率组成，其中第一个分类（索引号为5）是person
        19为输出的特征图大小，因为yolo的格子数一般默认定义为19x19
        '''
        if YOLOoutput.dim() == 3:
            # 网络的接收输入是一个mini-batch，image unsqueeze 后第一个维度是留给batch size的
            YOLOoutput = YOLOoutput.unsqueeze(0)    # 增加维度（0表示，在第一个位置增加维度）
        batch = YOLOoutput.size(0)      # 第0维度的数据数量
        ...
        # 分类置信度（pc）和一个分类（此处是human）的预测概率 乘取最大值就是需要的抗样本的dev_loss
...
class PatchApplier(nn.Module):
    """PatchApplier: applies adversarial patches to images. 图片上添加补丁
