...
baseFile_model_NAS   = './NAS/NAS-FCOS'
baseFile_model_AA    = './AA/adversarial-yolo-master'
...
for i in range(1, 1+iter_times):
    ...
    while True:
        # 执行Python文件
        result = subprocess.run(['python3', 'run_NAS.py', this_dataset_NAS],
                                capture_output=True)
        # 检查子进程的返回值
        if result.returncode == 0:
            print('子进程执行成功', i)
    ...
    while True:
        # 执行Python文件
        result = subprocess.run(['python3', 'train_NAS_result.py', this_dataset_NAS],
                                capture_output=True)
        # 检查子进程的返回值
        if result.returncode == 0:
            print('子进程执行成功', i)
